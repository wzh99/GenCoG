...
from .layer_matching import LayerMatching
from .mutation_utils import *
from ..logger.lemon_logger import Logger
...
def _shuffle_conv2d(weights, mutate_ratio):
    ...
    for val in weights:
        ...
        if len(val.shape) > 1:
            ...
            mutate_output_channels = utils.ModelUtils.generate_permutation(num_of_output_channels,
                                                                           mutate_ratio)
...
def GF_mut(model, mutation_ratio, distribution='normal', STD=0.1, lower_bound=None,
           upper_bound=None):
...
def WS_mut(model, mutation_ratio, mutated_layer_indices=None):
    ...
    mutated_layer_indices = np.arange(
        depth_layer) if mutated_layer_indices is None else mutated_layer_indices
            ...
            if len(weights) > 0:
                if isinstance(weights, list):
                    ...
                    if len(weights) != 2:
                        ...
                        permutation = utils.ModelUtils.generate_permutation(weights_w.shape[0],
                                                                            mutation_ratio)
...
def LA_mut(model, new_layers=None, mutated_layer_indices=None):
    ...
    layer_index_to_insert = layers_index_avaliable[
        np.random.randint(0, len(layers_index_avaliable))]
    ...
    mylogger.info('insert {} after {}'.format(layer_name_to_insert,
                                              LA_model.layers[layer_index_to_insert].name))
    # insert new layer
    if model.__class__.__name__ == 'Sequential':
        ...
        for i, layer in enumerate(LA_model.layers):
            ...
            if i == layer_index_to_insert:
                ...
                new_model.add(
                    layer_utils.available_model_level_layers[layer_name_to_insert](output_shape))
    else:
    ...
    for layer_name in old_model_layers.keys():
        ...
        for sw, w in zip(new_model_layers[layer_name].weights, layer_weights):
            ...
            for i in range(len(shape_sw)):
                assert shape_sw[i] == shape_w[i], '{}:{}/{}'.format(layer_name, shape_sw, shape_w)
            tuples.append((sw, w))
...
def MLA_mut(model, new_layers=None, mutated_layer_indices=None):
    ...
    available_new_layers = insertion_points[layer_index_to_insert]
    layer_name_to_insert = available_new_layers[np.random.randint(0, len(available_new_layers))]
    mylogger.info('choose to insert {} after {}'.format(layer_name_to_insert, MLA_model.layers[
        layer_index_to_insert].name))
    # insert new layers
    if model.__class__.__name__ == 'Sequential':
        import keras
        new_model = keras.models.Sequential()
...
def LS_mut(model):
    ...
    mylogger.info(
        'choose to swap {} ({} - {}) and {} ({} - {})'.format(layers[choose_index[0]].name,
