...
class LayerMatching:
    ...
    def __init__(self):
        ...
        self.input_legal[
            'global_max_pooling_1d'] = LayerMatching.global_pooling_1d_dense_input_legal
        self.layer_concats[
            'global_average_pooling_1d'] = LayerMatching.global_average_pooling_1d_dense
        self.input_legal[
            'global_average_pooling_1d'] = LayerMatching.global_pooling_1d_dense_input_legal
        self.layer_concats['global_max_pooling_2d'] = LayerMatching.global_max_pooling_2d_dense
        self.input_legal[
            'global_max_pooling_2d'] = LayerMatching.global_pooling_2d_dense_input_legal
        self.layer_concats[
            'global_average_pooling_2d'] = LayerMatching.global_average_pooling_2d_dense
        self.input_legal[
            'global_average_pooling_2d'] = LayerMatching.global_pooling_2d_dense_input_legal
        self.layer_concats['global_max_pooling_3d'] = LayerMatching.global_max_pooling_3d_dense
        self.input_legal[
            'global_max_pooling_3d'] = LayerMatching.global_pooling_3d_dense_input_legal
        self.layer_concats[
            'global_average_pooling_3d'] = LayerMatching.global_average_pooling_3d_dense
        self.input_legal[
            'global_average_pooling_3d'] = LayerMatching.global_pooling_3d_dense_input_legal
    ...
    def cropping1d_dense_input_legal(input_shape):
        input_shape = input_shape.as_list()
        return len(input_shape) == 3 and input_shape[0] is None and input_shape[1] is not None and \
               input_shape[1] > 2 \
               and input_shape[2] is not None and input_shape[1] * input_shape[
                   2] <= LayerMatching.concat_size_limit
    ...
    def cropping2d_dense(input_shape):
        ...
        layer_concat.append(
            keras.layers.Reshape(((input_shape[1] - 2) * (input_shape[2] - 2) * input_shape[3],)))
    ...
    def cropping2d_dense_input_legal(input_shape):
        ...
        return len(input_shape) == 4 and input_shape[0] is None \
               ...
               and input_shape[1] * input_shape[2] * input_shape[
                   3] <= LayerMatching.concat_size_limit
    ...
    def cropping3d_dense(input_shape):
        ...
        layer_concat.append(
            keras.layers.Dense(input_shape[1] * input_shape[2] * input_shape[3] * input_shape[4]))
        layer_concat.append(keras.layers.Reshape(input_shape[1:]))
        return layer_concat
    ...
    def cropping3d_dense_input_legal(input_shape):
        ...
        return len(input_shape) == 5 and input_shape[0] is None \
               ...
               and input_shape[1] * input_shape[2] * input_shape[3] * input_shape[
                   4] <= LayerMatching.concat_size_limit
    ...
    def upsampling_2d_dense_input_legal(input_shape):
        input_shape = input_shape.as_list()
        return len(input_shape) == 4 and input_shape[0] is None \
               and input_shape[1] is not None and input_shape[2] is not None and input_shape[
                   3] is not None \
               and input_shape[1] * input_shape[2] * input_shape[
                   3] <= LayerMatching.concat_size_limit
    ...
    def conv_lstm_2d_dense(input_shape):
        ...
        layer_concat = []
        layer_concat.append(
            keras.layers.ConvLSTM2D(input_shape[-1], kernel_size=(1, 1), strides=(1, 1),
