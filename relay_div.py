...
def main():
    ...
    div_record = []
    if args.generator == 'gencog':
        trend_file = time.strftime(f'out/gencog-%Y%m%d-%H%M%S.txt')
    else:
        trend_file = time.strftime(f'out/graphfuzz-{args.model}-%Y%m%d-%H%M%S.txt')
    loop_idx = 0
    while True:
        # Generate graph
        graph = gen.generate()
        ...
        if args.trend and loop_idx % 10 == 0:
