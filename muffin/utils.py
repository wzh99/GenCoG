...
def construct_layer_name(layer_id, layer_type, cell_type=''):
    return str(layer_id).zfill(2) + '_' + layer_type + (
            ('' if cell_type == '' else '_') + cell_type)
...
def get_layer_func(layer_type):
    ...
    return keras.Input if layer_type == 'input_object' else getattr(keras.layers,
                                                                    _str2Str(layer_type))
...
layer_conditions = {
    **{layer_name: activation_cond for layer_name in activation_layer_types},
    ...
    'bidirectional': (
        lambda **kwargs: kwargs.get('input_dim', None) == 3 or kwargs.get('input_dim', None) == 5),
    ...
    'dot': (lambda **kwargs: kwargs.get('input_num', None) == 2 and kwargs.get('output_shape',
                                                                               None) is None),
