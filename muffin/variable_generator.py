...
class VariableGenerator(object):
    ...
    def shape(self, dim: Optional[int] = None) -> Tuple[Optional[int], ...]:
        '''随机返回一个可以表示shape的元组
        '''
        if dim is None:
        ...
        return (
            ...
            *tuple(random.choices(
                range(self.__tensor_ele_size_range[0], self.__tensor_ele_size_range[1] + 1),
                k=dim - 1))
        )
    ...
    def dot_shapes(self):
        ...
        axes = (self.axis(len(input_shape_1)),
                self.axis(len(input_shape_2))) if self.boolean() else self.axis(
            min(len(input_shape_1), len(input_shape_2)))
