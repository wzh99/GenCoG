...
class OutputShapeCalculator(object):
    ...
    def conv_layer(self, input_shape, dim_num, data_format, kernel_size, dilation_rate, padding,
                   strides, filters, **kwargs):
        ...
        new_steps = [(old_steps[i] + plus[i] - window_len[i]) // strides[i] + 1 for i in
                     range(dim_num)]
        return (*input_shape[:-1 - dim_num], *new_steps, filters) if is_channels_last else (
            *input_shape[:-dim_num - 1], filters, *new_steps)
    ...
    def depthwise_conv2D_layer(self, input_shape, data_format, depth_multiplier, **kwargs):
        ...
        return self.conv_layer(input_shape, 2, data_format, filters=filters_in * depth_multiplier,
    ...
    def conv_transpose_layer(self, input_shape, dim_num, data_format, strides, kernel_size, padding,
                             filters, **kwargs):
        is_channels_last = (data_format == "channels_last")
        old_steps = input_shape[-1 - dim_num:-1] if is_channels_last else input_shape[-dim_num:]
        new_steps = [self.deconv_length(old_steps[i], strides[i], kernel_size[i], padding) for i in
                     range(dim_num)]
        return (*input_shape[:-1 - dim_num], *new_steps, filters) if is_channels_last else (
            *input_shape[:-dim_num - 1], filters, *new_steps)

    def pooling_layer(self, input_shape, dim_num, pool_size, strides, padding, data_format,
                      **kwargs):
        is_channels_last = (data_format == "channels_last")
        old_steps = input_shape[-1 - dim_num:-1] if is_channels_last else input_shape[-dim_num:]
        plus = [0 if padding == 'valid' else pool_size[i] - 1 for i in range(dim_num)]
        strides = pool_size if strides is None else strides
        new_steps = [(old_steps[i] + plus[i] - pool_size[i]) // strides[i] + 1 for i in
                     range(dim_num)]
        return (*input_shape[:-1 - dim_num], *new_steps, input_shape[-1]) if is_channels_last else (
            *input_shape[:-dim_num], *new_steps)
    ...
    def global_pool_layer(self, input_shape, dim_num, data_format, **kwargs):
        is_channels_last = (data_format == "channels_last")
        return (*input_shape[:-1 - dim_num], input_shape[-1]) if is_channels_last else input_shape[
                                                                                       :-dim_num]
    ...
    def bidirectional_layer(self, inner_layer_output_shape, merge_mode, **kwargs):
        return (*inner_layer_output_shape[:-1], 2 * inner_layer_output_shape[
            -1]) if merge_mode == 'concat' else inner_layer_output_shape
    ...
    def convLSTM2D_layer(self, input_shape, return_sequences, **kwargs):
        ...
        return (*input_shape[:2], *conv_output_shape[1:]) if return_sequences else (
            *input_shape[:1], *conv_output_shape[1:])
    ...
    def flatten_layer(self, input_shape):
        return (None, reduce(lambda x, y: x * y, input_shape[1:])) if len(input_shape) >= 2 else (
            None, 1)
    ...
    def locally_connected_layer(self, input_shape, dim_num, data_format, kernel_size, padding,
                                strides, filters, **kwargs):
        is_channels_last = (
                dim_num == 1 or data_format == "channels_last")  # dim_num=1时貌似忽略data_format参数
        old_steps = input_shape[-1 - dim_num:-1] if is_channels_last else input_shape[-dim_num:]
        plus = [0 if padding == 'valid' else kernel_size[i] - 1 for i in range(dim_num)]
        new_steps = [(old_steps[i] + plus[i] - kernel_size[i]) // strides[i] + 1 for i in
                     range(dim_num)]
        return (*input_shape[:-1 - dim_num], *new_steps, filters) if is_channels_last else (
            *input_shape[:-dim_num - 1], filters, *new_steps)
