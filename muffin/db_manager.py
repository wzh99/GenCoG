...
class DbManager(object):
    ...
    def get_incons_inputs_by_model_id_and_bk(self, model_id: int, backend: str, threshlod: float):
        SELECT_INCONS_INPUTS_AND_BKS_BY_MODEL_ID = '''select distinct input_index
                                                   ...
                                                   '''
        cur = self.__conn.cursor()
        cur.execute(SELECT_INCONS_INPUTS_AND_BKS_BY_MODEL_ID,
                    (model_id, '%' + backend + '%', threshlod,))
        return [res[0] for res in cur.fetchall()]

    def get_incons_bk_pairs_by_model_id_and_inputs(self, model_id: int, input_index: int,
                                                   threshold: float):
    ...
    def add_training_incons(self, model_id, backend_pair, model_output_delta, loss_delta,
                            loss_grads_delta):
        INSERT_INCONS = '''insert into inconsistency(model_id, backend_pair, model_output_delta, loss_delta, loss_grads_delta)
                           values(?, ?, ?, ?, ?)
                        '''
        self.__conn.execute(INSERT_INCONS, (
        model_id, backend_pair, model_output_delta, loss_delta, loss_grads_delta,))
        self.__conn.commit()
