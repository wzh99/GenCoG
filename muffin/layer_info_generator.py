...
from .utils import seq_layer_types, RNN_layer_types, activation_layer_types, merging_layer_types
...
class LayerInfoGenerator(object):
    ...
    def __init__(self, variable_generator: VariableGenerator, selector: Roulette):
        ...
        self.__layer_funcs = {name: getattr(self.layer_infos, name + '_layer') for name in
                              layer_types}

        # Roulette处理器
        self.__selector = selector

    def generate(self, input_shape: Tuple[Optional[int]], last_layer: Optional[str] = None,
    ...
    def generate_merging_layer(self, input_num: int = 2, pool: Optional[list] = None,
                               output_shape: Optional[tuple] = None):
        '''随机返回一个merging layer的name、参数字典、期望输入形状列表、理论输出形状
        '''
        pool = list(
            set(pool) & set(merging_layer_types)) if pool is not None else merging_layer_types
...
class LayerInfo(object):
    ...
    def conv1D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'conv1D', args, self.__output_shape.conv_layer(input_shape=input_shape, dim_num=1,
                                                              **args)
    ...
    def conv2D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'conv2D', args, self.__output_shape.conv_layer(input_shape=input_shape, dim_num=2,
                                                              **args)
    ...
    def conv3D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'conv3D', args, self.__output_shape.conv_layer(input_shape=input_shape, dim_num=3,
                                                              **args)
    ...
    def separable_conv1D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'separable_conv1D', args, self.__output_shape.conv_layer(input_shape=input_shape,
                                                                        dim_num=1, **args)
    ...
    def separable_conv2D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'separable_conv2D', args, self.__output_shape.conv_layer(input_shape=input_shape,
                                                                        dim_num=2, **args)
    ...
    def depthwise_conv2D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'depthwise_conv2D', args, self.__output_shape.depthwise_conv2D_layer(
    ...
    def conv2D_transpose_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'conv2D_transpose', args, self.__output_shape.conv_transpose_layer(
    ...
    def conv3D_transpose_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'conv3D_transpose', args, self.__output_shape.conv_transpose_layer(
    ...
    def max_pooling2D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        args = dict(
            ...
            strides=self.__random.sizes_with_limitation(
                window_limitation) if self.__random.boolean() else None,
            padding=self.__random.choice(["valid", "same"]),
            data_format="channels_last" if is_channels_last else "channels_first",
        )
        return 'max_pooling2D', args, self.__output_shape.pooling_layer(input_shape=input_shape,
                                                                        dim_num=2, **args)
    ...
    def max_pooling3D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'max_pooling3D', args, self.__output_shape.pooling_layer(input_shape=input_shape,
                                                                        dim_num=3, **args)
    ...
    def average_pooling2D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'average_pooling2D', args, self.__output_shape.pooling_layer(input_shape=input_shape,
                                                                            dim_num=2, **args)
    ...
    def average_pooling3D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'average_pooling3D', args, self.__output_shape.pooling_layer(input_shape=input_shape,
                                                                            dim_num=3, **args)
    ...
    def time_distributed_layer(self, input_shape: Tuple[Optional[int]]):
        '''只允许输入3+D向量
        '''
        inner_layer_type, inner_layer_args, inner_layer_output_shape = self.__generator.generate(
            (input_shape[0], *input_shape[2:],))
        args = dict(
            layer=dict(type=inner_layer_type, args=inner_layer_args)  # 由json生成模型时注意递归生成
        )
        return 'time_distributed', args, self.__output_shape.time_distributed_layer(
            input_shape=input_shape, inner_layer_output_shape=inner_layer_output_shape)

    def bidirectional_layer(self, input_shape: Tuple[Optional[int]]):
        '''包装RNN层
        '''
        inner_layer_type, inner_layer_args, inner_layer_output_shape = self.__generator.generate_RNN_layer(
            input_shape)
        args = dict(
            layer=dict(type=inner_layer_type, args=inner_layer_args),
            merge_mode=self.__random.choice(['sum', 'mul', 'concat', 'ave']),
            # None的话会返回list，链式模型中不适合
            weights=None,  # 未知用途的参数
        )
        return 'bidirectional', args, self.__output_shape.bidirectional_layer(
            inner_layer_output_shape=inner_layer_output_shape, **args)

    def convLSTM2D_layer(self, input_shape: Tuple[Optional[int]]):
        '''只允许输入5D向量
        '''
        kernel_size, strides, data_format, dilation_rate = self.__random.conv_args(
            (*input_shape[:1], *input_shape[2:]), dim_num=2)
        args = dict(
            filters=self.__random.ele_size(),
            kernel_size=kernel_size,
            strides=strides,
        ...
        return 'convLSTM2D', args, self.__output_shape.convLSTM2D_layer(input_shape=input_shape,
                                                                        **args)
    ...
    def batch_normalization_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'batch_normalization', args, self.__output_shape.batch_normalization_layer(
            input_shape=input_shape)

    def reshape_layer(self, input_shape: Tuple[Optional[int]],
    ...
    def repeat_vector_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'repeat_vector', args, self.__output_shape.repeat_vector_layer(
            input_shape=input_shape, **args)
    ...
    def cropping2D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'cropping2D', args, self.__output_shape.cropping_layer(input_shape=input_shape,
                                                                      dim_num=2, **args)
    ...
    def cropping3D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'cropping3D', args, self.__output_shape.cropping_layer(input_shape=input_shape,
                                                                      dim_num=3, **args)
    ...
    def locally_connected1D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'locally_connected1D', args, self.__output_shape.locally_connected_layer(
            input_shape=input_shape, dim_num=1, **args)
    ...
    def locally_connected2D_layer(self, input_shape: Tuple[Optional[int]]):
        ...
        return 'locally_connected2D', args, self.__output_shape.locally_connected_layer(
            input_shape=input_shape, dim_num=2, **args)

    def ReLU_layer(self, input_shape: Tuple[Optional[int]]):
        args = dict(
            max_value=self.__random.val_size(
                must_positive=True) if self.__random.boolean() else None,
        )
    ...
    def PReLU_layer(self, input_shape: Tuple[Optional[int]]):
        args = dict(
            ...
            shared_axes=self.__random.axis_list(
                len(input_shape)) if self.__random.boolean() else None,
        )
    ...
    def average_layer(self, input_num: int, output_shape):
        input_shape_list, output_shape, _ = self.__random.normal_merge_shapes(input_num=input_num,
                                                                              output_shape=output_shape)
